---
title: "S2208 MATH8050 Data Analysis - Section 001: Homework 7 Due on 10/26/22"
...
for(i in 1:n){
  ...
  ratio=min(1,(f(present_value)/dbeta(present_value,2,6))/
              (f(previous)/dbeta(previous,2,6)))
...
for (i in 1:n){
  ...
  ratio=min(1,(v(4,present_value)/v(2,present_value))/
              (v(4,previous)/v(2,previous)))
  accept = runif(1) < ratio
  stored[i]<-ifelse(accept,present_value,previous)
  previous=stored[i]
  if(accept){
