---
title: "S2208 MATH8050 Data Analysis - Section 001: Homework 5 Due on 10/13/22"
...
## 1a
```{r}
gen_lm <- glm(formula = Direction ~ Lag1+Lag2+Lag3+Lag4+Lag5+Volume,
...
```{r}
logres = function(X,y,threshold = 1e-10, max_iter = 100) {
  func1 = function(X,beta){
    beta = as.vector(beta)
...
dataframe1<-Smarket
dataframe1<-transform(dataframe1, new_d=ifelse(Direction=="Up",1,0))
...
my.predict= function(test_direction,values_p){ 
  test_data_direction<-c(rep(length(test_direction),0)) 
  c=1
  for (i in values_p){
    if (i>0.5){ 
...
Therefore we can see that false positive and false negative are 75 and 430 respectively.
We can verify that using predict function in r
...
\]
Now replace the $-x^4=t$ thereby, $-t^{1/4}=x$
\[dx=\frac{1}{4}t^{-3/4}dt
\]
Now writing the equation w.r.t t, we get:
\[2\int_{0}^{\infty}exp(-x^4)=2/4\int_{0}^{\infty}t^{\frac{1}{4}-1}exp(-t)dt
\]
Which is in the gamma function form,
\[\Gamma(z)=\int_{0}^{\infty}exp(-t)t^{z-1}dt
\]
where \[z= 1/4 \Gamma(1/4)=3.6356\]
thereby, \[0.5 \int_{0}^{\infty}t^{\frac{1}{4}-1}exp(-t)dt=\Gamma(1/4)0.5 = 1.8128
\]
...
```{r}
h = function(x){
  return(2*exp(-(x^4))/dgamma(x,shape = 1.2,rate=0.95))
}
...
c(mean(h(x)),var(h(x)))
```
As the above integration limits follow the limits of Gamma Distribution so we have taken the Gamma
Distribution with shape and rate as 1.2 and 0.95.
...
set.seed(29) 
w<-function(x) 
dgamma(x,shape=1.2,rate = 0.95)/dexp(x,2.2) 
X<-rexp(10^5,2.2) 
Y<-w(X)*h(X)
c(mean(Y),var(Y))
```
With the help of sampling we can see the variance of the value is reduced and also the precision of the value
is increased at rate = 2.2.
